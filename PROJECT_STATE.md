...
### ✅ 真机 POC 从「人肉观察」改成自动测量（2026-09-06）

M1 唯一剩下的事是真机 POC，而它原本是「人肉观察九项指标再手填表格」。
那样的数据不可比 ——「有点漂」没法拿去和另一个 provider 比，
也没法在调完滤波参数后判断到底变好了没有。

**漂移现在能算了，而且不需要任何真值标注。**
anchor 在脸部局部坐标系里的位置理论上恒定（坐标系以双眼为原点、瞳距为单位，
对远近/平移/歪头天然不变 —— golden vector 已离线证明到 2e-15）。
所以真人脸上它的残余波动就是漂移本身。
...
| 指标 | 含义 | 用户看到 | 调哪个参数 |
| --- | --- | --- | --- |
| drift | 相对整段均值的偏离 | 点慢慢跑偏 | `beta` 调大 |
| jitter | 相邻帧位移 | 点在原地抖 | `minCutoff` 调小 |

单位都是瞳距，所以不同的人、不同距离、不同机型之间可以直接比。

- **`StabilityMeter` / `POCRecorder`（Core，纯 Swift）** —— 期望值先在 Python 里
  独立算过再写进 Swift 断言，和 golden vector 一个路子；
  否则测试只会证明"代码等于它自己"。
- **App 内 POC 测量台**（Settings → Developer → Diagnostics → POC 测量）：
  14 个预置场景，选场景 → 保持姿势 → 停止 → 导出 Markdown 表格 + JSON。
  标签固定不让人自由填 —— 自由填的结果是三个人测出三套对不上的数据。
- **浏览器原型也带了同一套测量** —— 不用等 iPhone，在 Windows 上今天就能
  拿到第一组真人脸数据。`tools/check_prototype_math.py` 在 CI 上交叉验证
  JS 与 Python 参考实现一致（4 组用例，容差 1e-9），否则两边数据不可比。

刻意的两处克制：
- **不给「合格/不合格」**，只给 `漂移/容差` 比值。阈值定在哪是规格 §18 的
  Go/No-Go 判定，得先有数据。
- **手遮挡那条验收红线仍然由人看。**「屏幕上有没有让用户觉得被评判的东西」
  是观感问题，测量台答不了。文档里写明了这一点，免得有人以为跑完工具就算测完。

`lost` 帧不计入位置统计（那些帧位置本来就没意义，混进去会让漂移虚高、
反而掩盖真实问题），但仍计入质量分布。

### 🧰 这一轮新增的三条检查（都是我自己刚犯过的错）

| 规则 | 起因 | 反向验证 |
| --- | --- | --- |
| 网站文案无功效表述 | 要在官网写文章引流，营销文案天然往承诺上滑 | 规则写完当场咬到我自己写的两句话；另塞 "Firming results you can see." 被拦下 |
| 重复声明属性 | 我给控制器加访问器时写了个和已有 `lockLossCount` 重名的属性 | 第一版报了 6 个误报（没区分方法重载与属性），缩到只查属性后 0 误报、自测命中 |
| 字符串字面量闭合 | heredoc 把 `\n` 写成真换行，字面量跨了三行 | 把 CI 上真实炸掉的形态塞回去，报在第 48/50 行 —— 与 Xcode 报的位置完全一致 |

「括号配对」为什么没拦住第三条：它的扫描器遇到未闭合引号会一路把后面内容
当成字符串吞掉，括号数恰好还是平的 —— 靠运气蒙混过去了。
**检查器沉默不等于代码没问题**，这也是每条规则都必须反向验证的原因。
...
- `python tools/check_prototype_math.py` → 浏览器版统计与 Python 参考实现 4 组用例全部一致（最大偏差 1.6e-15）
